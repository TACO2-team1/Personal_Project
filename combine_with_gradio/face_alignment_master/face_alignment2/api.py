...
class FaceAlignment:
    def __init__(self, landmarks_type, network_size=NetworkSize.LARGE,
        ...
        face_detector_module = __import__('face_alignment2.detection.' + face_detector,
    ...
    # @torch.no_grad()
    def get_landmarks_from_image_pgd(self, image_or_path, detected_faces=None, return_bboxes=False,
                                 return_landmark_score=False):
        image=image_or_path
        ...
        landmarks_scores = []
        for i, d in enumerate(detected_faces):
            # Using PyTorch operations for center and scale calculation
            center = torch.tensor([d[2] - (d[2] - d[0]) / 2.0, d[3] - (d[3] - d[1]) / 2.0], requires_grad=True)
            center[1] = center[1] - (d[3] - d[1]) * 0.12
            scale = (d[2] - d[0] + d[3] - d[1]) / self.face_detector.reference_scale

            # Crop using PyTorch tensor
            inp = crop(image, center, scale)
            inp = torch.tensor(inp.transpose((2, 0, 1)), dtype=torch.float32, requires_grad=True)

            inp = inp.to(self.device, dtype=self.dtype)
            inp.div_(255.0).unsqueeze_(0)  # Normalize the image

            # Forward pass for prediction
            out = self.face_alignment_net(inp)  # Removed .detach() to keep the computation graph
            if self.flip_input:
            ...
            # Get predictions from heatmap
            pts, pts_img, scores = get_preds_fromhm(out, center, scale)

            # Convert to PyTorch tensors, maintaining the computation graph
            pts, pts_img = torch.tensor(pts, dtype=torch.float32, requires_grad=True), torch.tensor(pts_img, dtype=torch.float32, requires_grad=True)
            pts, pts_img = pts.view(68, 2) * 4, pts_img.view(68, 2)
            scores = scores.squeeze(0)

            landmarks.append(pts_img)  # Directly append tensors
            landmarks_scores.append(scores)
