...
#     return bboxlists
def batch_detect(net, img_batch, device):
    """Batch detection using the model."""
    if 'cuda' in device:
        torch.backends.cudnn.benchmark = True
    batch_size = img_batch.size(0)
    img_batch = img_batch.to(device, dtype=torch.float32)
    ...
    olist = net(img_batch)  # Model forward pass

    for i in range(len(olist) // 2):
        olist[i * 2] = F.softmax(olist[i * 2], dim=1)

    # Ensure all outputs remain tensors on the same device
...
def get_predictions(olist, batch_size, device):
    """Generate predictions from network output."""
    bboxlists = []
    variances = [0.1, 0.2]
    for i in range(len(olist) // 2):
        ocls, oreg = olist[i * 2], olist[i * 2 + 1]
        stride = 2**(i + 2)  # 4, 8, 16, 32, 64, 128
        poss = torch.nonzero(ocls[:, 1, :, :] > 0.05, as_tuple=False)
        for Iindex, hindex, windex in poss:
            axc, ayc = stride / 2 + windex * stride, stride / 2 + hindex * stride
            priors = torch.tensor([[axc / 1.0, ayc / 1.0, stride * 4 / 1.0, stride * 4 / 1.0]], device=device)
            score = ocls[:, 1, hindex, windex].unsqueeze(-1)
            loc = oreg[:, :, hindex, windex]
            boxes = decode(loc.unsqueeze(0), priors, variances)
            bboxlists.append(torch.cat((boxes, score), dim=1))
    
    if len(bboxlists) == 0:  # No candidates within the given threshold
        bboxlists = [torch.empty((0, 5), device=device) for _ in range(batch_size)]
    else:
        bboxlists = torch.stack(bboxlists, dim=1)
    return bboxlists
