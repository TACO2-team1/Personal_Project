...
def pgd_attack(model, x, epsilon, alpha, num_iter, tv_weight=tv_weight):
    x_adv = tf.identity(x)
    ...
    for _ in range(num_iter):
        with tf.GradientTape() as tape:
            ...
            print(bbox_regress)
            output_face_classifier = output_classifier[0,...,0]
            
            # 组合损失：对抗损失 + TV损失
            adv_loss = bce_loss(tf.zeros_like(output_face_classifier), output_face_classifier)
            if tv_loss:
...
def multiple_pgd_attack(img, epsilon=epsilon, alpha=alpha, num_iter=num_iter,img_ct=img_ct):
    ...
    for scale in scales:
        ...
        delta = pgd_attack(Pnet, caffe_img_tensor, epsilon, alpha, num_iter)
...
img = cv2.imread('result_img/5.png')
adv_img_bgr = multiple_pgd_attack(img)
# adv_img_rgb = cv2.cvtColor(adv_img_bgr, cv2.COLOR_RGB2BGR)

threshold = [0.6, 0.6, 0.7]
if save_img is True:
    cv2.imwrite(f'result_img/tv_adv_img_{img_ct}_{epsilon}_{alpha}_{num_iter}.jpg', adv_img_bgr)
    for i in range(1):
        ...
        cv2.imwrite(
            f'result_img/tv_adv_img_predicted_{img_ct}_{epsilon}_{alpha}_{num_iter}_{tv_weight}_.jpg', draw)
